...
$(document).ready(function() {
	...
	image.src = 'images/spood5.png';

	canvas = document.createElement('canvas');
	$(canvas).attr('width', 500);
	$(canvas).attr('height', 500);
	$('#skew-frame').append(canvas);
	...
	$('#skew-frame').mousedown(function(e) {
		if ($(e.target).hasClass('node')) {
			var node = e.target;
			var frameOffset = $('#skew-frame').offset();

			$('#skew-frame').mousemove(function(e) {
				var x = e.pageX - frameOffset.left - 12;
				var y = e.pageY - frameOffset.top - 12;
				$(node).css('left', x);
				$(node).css('top', y);
				dirtyTriangles = true;
			});

			$('#skew-frame').mouseup(function(e) {
				$('#skew-frame').off("mousemove");
				$('#skew-frame').off("mouseup");
			});
		}
	});
});
